...
Core::User::User(int fd)
    : _fd(fd), _quitMessage("Left the chat"), _hasNick(false), _hasUser(false), _hasSentPassword(false), _isValidPassword(false), _disconnected(true), _capabilitiesNegotiationFinished(true), _toDelete(false)
{
    ResolveHostname();
...
bool Core::User::ReadyToSend(void)
{
    return _outgoingBuffer.size() > 0;
}

void Core::User::SetIncomingBuffer(const std::string &from)
{
...
void Core::User::AppendToOutgoingBuffer(const std::string &from)
{
    std::string buf(from);
    if (from.find("\r\n") == from.npos)
        buf += "\r\n";
...
void Core::User::AppendToOutgoingBuffer(const char *from)
{
    std::string buf(from);
    if (buf.find("\r\n") == buf.npos)
        buf += "\r\n";
...
int Core::User::Fileno(void)
{
    return _fd;
}

bool Core::User::FullyRegistered(void)
{
    return _hasNick && _hasUser && HasFinishedCapNeg() && HasSentValidPassword();
}

void Core::User::SetOutgoingBuffer(const char *from)
{
    _outgoingBuffer = from;
}

void Core::User::SetOutgoingBuffer(const std::string &from)
{
    _outgoingBuffer = from;
}
...
void Core::User::ClearOutgoingBuffer(void)
{
    _outgoingBuffer.clear();
}

void Core::User::ClearIncomingBuffer(void)
{
    _incomingBuffer.clear();
}

void Core::User::SetNickname(const std::string &from)
{
    _nickname = from;
    _hasNick = true;
}
...
void Core::User::SetUsername(const std::string &from)
{
    _username = from;
    _hasUser = true;
}
...
void Core::User::SetRealName(const std::string &from)
{
    _realname = from;
}
...
void Core::User::FinishCapabilitiesNegotiation(void)
{
    _capabilitiesNegotiationFinished = true;
}

void Core::User::PendingCapabilitiesNegotiation(void)
{
    _capabilitiesNegotiationFinished = false;
}

bool Core::User::HasFinishedCapNeg(void)
{
    return _capabilitiesNegotiationFinished == true;
}

bool Core::User::CompletedRegistrationRoutine(const std::string &from)
{
...
void Core::User::SetPasswordReceived(bool state)
{
    _hasSentPassword = state;
}

bool Core::User::SetPasswordValid(bool state)
{
    _isValidPassword = state;
    return state;
}

bool Core::User::HasSentValidPassword(void)
{
    return _hasSentPassword && _isValidPassword;
}

void Core::User::ResolveHostname(void)
{
...
void Core::User::SetQuitMessage(const std::string &msg)
{
    _quitMessage = msg;
}
...
void Core::User::MarkForDeletion(void)
{
    _toDelete = true;
}

bool Core::User::MustBeDeleted(void)
{
    return _toDelete;
}

void Core::User::sendMotd(void)
{
    std::ifstream f;
