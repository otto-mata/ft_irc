...
int
Core::Arguments::atoi_overflow_guard(char *nptr, int *res) {
    int guard = 0;
    int sign = 1;
...
Core::Arguments::Arguments(int argc, char **argv) {
    _argc = argc;
    _argv = argv;
}

int
Core::Arguments::parseArgs(void) {
    Logging::Engine log("ArgParse");
    if (_argc < 3) {
        log.err("Usage: " + std::string(_argv[0]) + " <port> <password>");
        return 1;
    }
    if (_argc > 3)
        log.warn(
    ...
    if (!atoi_overflow_guard(_argv[1], &nPort)) {
        log.err("Invalid port number: integer overflow or wrong format.");
        return 2;
    }
    if (nPort > 65535 || nPort < 1) {
        log.err("Invalid port number: should be in range [0 - 65536[.");
        return 2;
    }
    port = nPort;
