...
int
Commands::Mode::handleOpMode(const triplet &t) {
    if (!t.HasValue)
        return Replies::ERR_NEEDMOREPARAMS(emitter, raw->Name());
...
int
Commands::Mode::handleKeyMode(const triplet &t) const {
...
int
Commands::Mode::handleLimMode(const triplet &t) const {
    if (t.Mode == '+' && !t.HasValue)
        return Replies::ERR_NEEDMOREPARAMS(emitter, raw->Name());
    if (t.Mode == '+') {
        size_t n = 0;
...
Commands::Mode::Execute(void) {
    ...
    if (raw->Arguments().size() == 1) {
        const std::string modes(targetChannel->GetChannelModes());
        emitter->AppendToOutgoingBuffer(
            ":" + ctx->Hostname() + " 324 " + emitter->GetNickname() + " #" + targetChannel->GetName() + " " + modes);
        return 0;
    }
    ...
    for (std::vector<triplet>::iterator it = modeUpdates.begin();
        ...
        if (it->Flag == 'i') {
            ...
            targetChannel->SetTopicModifiable(it->Mode != '+');
            targetChannel->Broadcast(":" + emitter->FullIdentityString() + " MODE #" +
                                     targetChannel->GetName() + " " + it->Mode +
                                     "t");
        } else if (it->Flag == 'k')
